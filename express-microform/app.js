...
var generateCaptureContext = require('./routes/micro').generate_capture_context_with_checkout_api;
...
app.get('/checkout', function (req, res) {
    generateCaptureContext(function (error, data) {
        if (error) {
            res.status(500).send('Error generating capture context');
        } else {
            res.render('index', { keyInfo: data.keyId });
        }
    });
});
...
app.post('/receipt', function (req, res) {
    ...
    try {
        ...
        clientReferenceInformation.code = 'hina_flex_payment';
